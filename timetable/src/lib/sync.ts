...
const CFG_KEY = 'academy-timetable/sync';
...
export async function fetchRemote(cfg: SyncConfig): Promise<RemoteBoard | null> {
  const res = await fetch(
    `${endpoint(cfg)}?id=eq.${encodeURIComponent(cfg.roomId)}&select=data,updated_at`,
    { headers: headers(cfg) },
  );
  if (!res.ok) throw new Error(`서버 응답 오류 (${res.status})`);
  const rows = (await res.json()) as { data: TimetableData; updated_at: string }[];
...
export async function fetchRemoteStamp(cfg: SyncConfig): Promise<string | null> {
  const res = await fetch(
    `${endpoint(cfg)}?id=eq.${encodeURIComponent(cfg.roomId)}&select=updated_at`,
    { headers: headers(cfg) },
  );
  if (!res.ok) throw new Error(`서버 응답 오류 (${res.status})`);
...
export async function pushRemote(cfg: SyncConfig, data: TimetableData): Promise<string> {
  const updatedAt = new Date().toISOString();
  const res = await fetch(`${endpoint(cfg)}?on_conflict=id`, {
    method: 'POST',
    headers: { ...headers(cfg), Prefer: 'resolution=merge-duplicates' },
    body: JSON.stringify({ id: cfg.roomId, data, updated_at: updatedAt }),
  });
